    ...
    class AsynchronousClient < Client
      ...
      def execute(cql, *args)
        with_failure_handler do
          ...
          execute_request(Protocol::QueryRequest.new(cql, args, options[:type_hints], options[:consistency], options[:serial_consistency], nil, nil, options[:trace]), options[:timeout])
        end
      end
