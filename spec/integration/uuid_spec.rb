...
describe 'Loading and storing UUIDs', :integration do
  ...
  context Cassandra::Uuid do
    ...
    it 'can be used to store data in UUID cells' do
      store_statement.execute(Cassandra::Uuid.new('39bc6ab8-d0f5-11e2-b041-adb2253022a3'), 'hello world')
    end

    it 'will be used when loading data from UUID cells' do
      store_statement.execute(Cassandra::Uuid.new('39bc6ab8-d0f5-11e2-b041-adb2253022a3'), 'hello world')
      client.execute(%<SELECT * FROM ids>).first['id'].should == Cassandra::Uuid.new('39bc6ab8-d0f5-11e2-b041-adb2253022a3')
    end

    it 'works even when the UUID could be represented as fewer than 16 bytes' do
      store_statement.execute(Cassandra::Uuid.new('00853800-5400-11e2-90c5-3409d6a3565d'), 'hello world')
    end
  end
