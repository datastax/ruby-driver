    ...
    describe ResultResponse do
      describe '.decode!' do
        it 'decodes a set_keyspace result' do
          response = described_class.decode!(1, ByteBuffer.new("\x00\x00\x00\x03\x00\x06system"))
          response.should_not be_void
          response.keyspace.should == 'system'
        end

        it 'decodes a schema_change CREATED result' do
          response = described_class.decode!(1, ByteBuffer.new("\x00\x00\x00\x05\x00\aCREATED\x00\ncql_rb_477\x00\x00"))
          response.should_not be_void
        ...
        it 'decodes a schema_change UPDATED result' do
          response = described_class.decode!(1, ByteBuffer.new("\x00\x00\x00\x05\x00\aUPDATED\x00\ncql_rb_973\x00\x05users"))
          response.should_not be_void
        ...
        it 'decodes a prepared result' do
          response = described_class.decode!(1, ByteBuffer.new("\x00\x00\x00\x04\x00\x10\xCAH\x7F\x1Ez\x82\xD2<N\x8A\xF35Qq\xA5/\x00\x00\x00\x01\x00\x00\x00\x01\x00\ncql_rb_911\x00\x05users\x00\tuser_name\x00\r"))
          response.should_not be_void
        ...
        it 'complains when asked to decode an unknown result type' do
          expect { described_class.decode!(1, ByteBuffer.new("\x00\x00\x00\xffhello")) }.to raise_error(UnsupportedResultKindError)
        end
      end
    end
  end
