    ...
    describe EventResponse do
      describe '.decode!' do
        it 'decodes a SCHEMA_CHANGE event' do
          response = described_class.decode!(1, ByteBuffer.new("\x00\rSCHEMA_CHANGE\x00\aDROPPED\x00\ncql_rb_609\x00\x05users"))
        ...
        it 'decodes a STATUS_CHANGE event' do
          response = described_class.decode!(1, ByteBuffer.new("\x00\rSTATUS_CHANGE\x00\x04DOWN\x04\x00\x00\x00\x00\x00\x00#R"))
        ...
        it 'decodes a TOPOLOGY_CHANGE event' do
          response = described_class.decode!(1, ByteBuffer.new("\x00\x0FTOPOLOGY_CHANGE\x00\fREMOVED_NODE\x04\x00\x00\x00\x00\x00\x00#R"))
        ...
        it 'complains when asked to decode an unknown event type' do
          expect { described_class.decode!(1, ByteBuffer.new("\x00\x04PING")) }.to raise_error(UnsupportedEventTypeError, /PING/)
        end
      end
    end
  end
