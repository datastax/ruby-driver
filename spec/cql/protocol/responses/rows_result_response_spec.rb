      ...
      describe '.decode!' do
        ...
        context 'with COUNTER columns' do
          it 'decodes COUNTER as a number' do
            response = described_class.decode!(1, ByteBuffer.new("\x00\x00\x00\x01\x00\x00\x00\x03\x00\x04test\x00\x04cnts\x00\x02id\x00\r\x00\x02c1\x00\x05\x00\x02c2\x00\x05\x00\x00\x00\x01\x00\x00\x00\x04theo\x00\x00\x00\b\x00\x00\x00\x00\x00\x00\x00\x03\x00\x00\x00\b\x00\x00\x00\x00\x00\x00\x00\x01"))
            response.rows.first['c1'].should == 3
          end

          it 'decodes a null COUNTER as nil' do
            response = described_class.decode!(1, ByteBuffer.new("\x00\x00\x00\x01\x00\x00\x00\x02\x00\x12cql_rb_client_spec\x00\bcounters\x00\bcounter1\x00\x05\x00\bcounter2\x00\x05\x00\x00\x00\x01\x00\x00\x00\b\x00\x00\x00\x00\x00\x00\x00\x01\xFF\xFF\xFF\xFF"))
        ...
        context 'with an unknown column type' do
          it 'raises an error when encountering an unknown column type' do
            ...
            expect { described_class.decode!(1, buffer) }.to raise_error(UnsupportedColumnTypeError)
          end
        end
      end
