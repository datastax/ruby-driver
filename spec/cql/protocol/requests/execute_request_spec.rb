    ...
    describe ExecuteRequest do
      ...
      describe '#initialize' do
        it 'raises an error when the metadata and values don\'t have the same size' do
          expect { ExecuteRequest.new(id, column_metadata, ['hello', 42], true, :each_quorum, false) }.to raise_error(ArgumentError)
        end

        it 'raises an error for unsupported column types' do
          column_metadata[2][3] = :imaginary
          expect { ExecuteRequest.new(id, column_metadata, ['hello', 42, 'foo'], true, :each_quorum, false) }.to raise_error(UnsupportedColumnTypeError)
        end
        ...
        it 'raises an error when collection values are not enumerable' do
          ...
          expect { ExecuteRequest.new(id, column_metadata, ['hello', 42, 'foo'], true, :each_quorum, false) }.to raise_error(InvalidValueError)
        end

        it 'raises an error when it cannot encode the argument' do
          expect { ExecuteRequest.new(id, column_metadata, ['hello', 'not an int', 'foo'], true, :each_quorum, false) }.to raise_error(TypeError, /cannot be encoded as INT/)
        end
      end
      ...
      describe '#write' do
        context 'when the protocol version is 1' do
          let :frame_bytes do
            ExecuteRequest.new(id, column_metadata, ['hello', 42, 'foo'], true, :each_quorum, false).write(1, '')
          end
        ...
        context 'when the protocol version is 2' do
          let :frame_bytes do
            ExecuteRequest.new(id, column_metadata, ['hello', 42, 'foo'], true, :each_quorum, false).write(2, '')
          end
          ...
          it 'does not write the bound values flag when there are no values, and does not write anything more' do
            frame_bytes = ExecuteRequest.new(id, [], [], true, :each_quorum, false).write(2, '')
            frame_bytes.to_s[20, 999].should == "\x00"
          end

          it 'writes flags saying that the result doesn\'t need to contain metadata' do
            frame_bytes = ExecuteRequest.new(id, column_metadata, ['hello', 42, 'foo'], false, :each_quorum, false).write(2, '')
        ...
        context 'with different data types' do
          ...
          specs.each do |type, value, expected_bytes|
            it "encodes #{type} values" do
              ...
              buffer = ExecuteRequest.new(id, metadata, [value], true, :one, false).write(1, ByteBuffer.new)
      ...
      describe '#to_s' do
        it 'returns a pretty string' do
          request = ExecuteRequest.new(id, column_metadata, values, true, :each_quorum, false)
      ...
      describe '#eql?' do
        it 'returns true when the ID, metadata, values and consistency are the same' do
          e1 = ExecuteRequest.new(id, column_metadata, values, true, :one, false)
          e2 = ExecuteRequest.new(id, column_metadata, values, true, :one, false)
          e1.should eql(e2)
        end

        it 'returns false when the ID is different' do
          e1 = ExecuteRequest.new(id, column_metadata, values, true, :one, false)
          e2 = ExecuteRequest.new(id.reverse, column_metadata, values, true, :one, false)
          e1.should_not eql(e2)
        end

        it 'returns false when the metadata is different' do
          e1 = ExecuteRequest.new(id, column_metadata, values, true, :one, false)
          e2 = ExecuteRequest.new(id, column_metadata.reverse, values, true, :one, false)
          e1.should_not eql(e2)
        end

        it 'returns false when the values are different' do
          e1 = ExecuteRequest.new(id, column_metadata, values, true, :one, false)
          e2 = ExecuteRequest.new(id, column_metadata, values.reverse, true, :one, false)
          e1.should_not eql(e2)
        end

        it 'returns false when the consistency is different' do
          e1 = ExecuteRequest.new(id, column_metadata, values, true, :one, false)
          e2 = ExecuteRequest.new(id, column_metadata, values, true, :two, false)
          e1.should_not eql(e2)
        end
