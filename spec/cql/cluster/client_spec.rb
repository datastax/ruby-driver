    ...
    describe(Client) do
      ...
      describe('#query') do
        it 'sends a QueryRequest' do
          ...
          client.connect.get
          client.query(Statements::Simple.new('SELECT * FROM songs'), :consistency => :one).get
        ...
        it 'switches keyspace of new connections automatically' do
          ...
          client.connect.get
          client.query(Statements::Simple.new('USE foo'), :consistency => :one).get
        ...
        it 'waits for keyspace to be switched before running other requests' do
          ...
          completed = 0
          5.times do
            f = client.query(Statements::Simple.new('SELECT * FROM songs'), :consistency => :one)
            f.map do |r|
              completed += 1
              r
            end
      ...
      describe('#prepare') do
        it 'sends a PrepareRequest' do
          ...
          client.connect.get
          statement = client.prepare('SELECT * FROM songs', {:consistency => :one}).get
      ...
      describe('#execute') do
        it 'sends an ExecuteRequest' do
          ...
          client.connect.get
          statement = client.prepare('SELECT * FROM songs', {:consistency => :one}).get
          client.execute(statement.bind, {:consistency => :one}).get
      ...
      describe('#batch') do
        it 'sends a BatchRequest' do
          ...
          client.batch(batch, :consistency => :one, :trace => false).get
          expect(sent).to be_true
        end
        ...
        it 'can include prepared statements' do
          ...
          statement = client.prepare('INSERT INTO songs (id, title, album, artist, tags) VALUES (?, ?, ?, ?, ?)', {:trace => false}).get
        ...
        it 'follows the plan on failure' do
          ...
          client.batch(batch, {:consistency => :one}).get
