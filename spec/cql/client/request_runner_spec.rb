    ...
    describe RequestRunner do
      ...
      describe '#execute' do
        ...
        let :raw_rows_response do
          Protocol::RawRowsResultResponse.new(2, Protocol::ByteBuffer.new("\x00\x00\x00\x02\x00\x00\x00\x01a\x00\x00\x00\x01b"), nil, nil)
        end
        ...
        context 'when the response has a paging state' do
          ...
          it 'returns a LazyQueryResult that knows its paging state' do
            metadata = [['ks', 'tbl', 'col', :varchar]]
            connection.stub(:send_request).with(request, anything).and_return(Future.resolved(Protocol::RawRowsResultResponse.new(2, Protocol::ByteBuffer.new("\x00\x00\x00\x02\x00\x00\x00\x01a\x00\x00\x00\x01b"), 'bazbuzz', nil)))
