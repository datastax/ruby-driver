    ...
    describe KeyspaceChanger do
      ...
      describe '#use_keyspace' do
        it 'sends a query request with a USE statement' do
          connection.stub(:send_request).with(Protocol::QueryRequest.new('USE important_stuff', nil, nil, :one), nil).and_return(Ione::Future.resolved)
        ...
        it 'accepts quoted keyspace names' do
          connection.stub(:send_request).with(Protocol::QueryRequest.new('USE "ImportantStuff"', nil, nil, :one), nil).and_return(Ione::Future.resolved)
