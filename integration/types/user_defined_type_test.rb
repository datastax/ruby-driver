...
class UserDefinedTypeTest < IntegrationTestCase
  ...
  def test_can_insert_udts_with_varying_lengths
    ...
    # Insert into table
    insert = Retry.with_attempts(5) { session.prepare("INSERT INTO simplex.mytable (zz, a) VALUES (?, ?)") }
  ...
  def test_can_insert_nested_udts
    ...
    [0, 1, 2, 3, max_udt_depth].each do |depth|
      input = udts[depth]
      insert = Retry.with_attempts(5) { session.prepare("INSERT INTO simplex.mytable (zz, v_#{depth}) VALUES (0, ?)") }
  ...
  def test_can_insert_udt_all_datatypes_nil
    ...
    DatatypeUtils.primitive_datatypes.zip('a'..'z').map do |_, letter|
