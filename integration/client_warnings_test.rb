...
batch for atomicity, or asynchronous writes for performance/)
  ...
  # Test for batch partition client warnings
  #
  # test_batch_two_partitions tests that client warnings related to batches covering multiple partitions are presented
  # by the driver.
  #
  # @since 3.0.0
  # @jira_ticket RUBY-133
  ...
  # @test_category queries:client_warning
  #
  def test_batch_two_partitions
    skip("Client warnings are only available in C* after 2.2") if CCM.cassandra_version < '2.2.0'

    cluster = Cassandra.cluster
    session = cluster.connect("simplex")

    insert = session.prepare("INSERT INTO simplex.test (k, v) VALUES (?, ?)")
    query = session.unlogged_batch do |batch|
      batch.add(insert, arguments: [0, "aa"])
      batch.add(insert, arguments: [1, "bb"])
    end
    execution_info = session.execute(query).execution_info

    assert_equal 1, execution_info.warnings.size
    assert_match @partition_warning, execution_info.warnings.first
  end

  # Test for batch partition client warnings, trace, and custom payload
  #
  # test_batch_two_partitions_with_trace_and_custom_payload tests that client warnings related to batches covering
  # multiple partitions are presented by the driver, even with tracing enabled and a custom payload.
  #
  # @since 3.0.0
  # @jira_ticket RUBY-133
  # @expected_result Client warnings should not empty, with tracing and a custom payload
  ...
  # @test_category queries:client_warning
  #
  def test_batch_two_partitions_with_trace_and_custom_payload
    skip("Client warnings are only available in C* after 2.2") if CCM.cassandra_version < '2.2.0'
